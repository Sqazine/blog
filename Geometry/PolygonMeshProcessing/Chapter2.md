...
![图2.1.三角形网格的面集数据结构(左)与索引面集数据结构(右).](Chapter2/image.png)
...
![图 2.2.存储在基于面的数据结构中的连通性信息.](Chapter2/image-1.png)
...
![图 2.3.基于边的数据结构中存储的连通性信息.](Chapter2/image-2.png)
...
![图 2.4. 半边数据结构中存储的连通性信息.](Chapter2/image-3.png)
...
![图 2.5.中心顶点的单环邻域可以通过从中心顶点的外射半边开始(左),然后通过步进到对半边(中)和下一个半边(右)顺时针重复旋转,直到再次到达第一个半边枚举得到.](Chapter2/image-4.png)
